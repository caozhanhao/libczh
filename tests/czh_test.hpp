...
namespace czh::test
{
  ...
  {
    LIBCZH_MARK("CZH")
    czh::Czh czh("../../tests/czh/inputtest.czh", czh::InputMode::nonstream);
    auto nodeptr = czh.parse();
    LIBCZH_EXPECT_FALSE("Node 1", nodeptr == nullptr);
    auto &node = *nodeptr;
    LIBCZH_EXPECT_EQ("DT1", node["czh"]["dt1"].get<double>(), 1.0000000000000002);
    LIBCZH_EXPECT_EQ("DT2", node["czh"]["dt2"].get<double>(), 2.2250738585072009e-308);
    LIBCZH_EXPECT_EQ("DT3", node["czh"]["dt3"].get<double>(), -2.2250738585072009e-308);
    LIBCZH_EXPECT_EQ("DT4", node["czh"]["dt4"].get<double>(), 2.2250738585072014e-308);
    LIBCZH_EXPECT_EQ("DT5", node["czh"]["dt5"].get<double>(), -2.2250738585072014e-308);
    LIBCZH_EXPECT_EQ("DT6", node["czh"]["dt6"].get<double>(), 1.7976931348623157e+308);
    LIBCZH_EXPECT_EQ("DT7", node["czh"]["dt7"].get<double>(), -1.7976931348623157e+308);
    LIBCZH_EXPECT_EQ("DT8", utils::dtoa(node["czh"]["dt8"].get<double>()), "0.0");
    LIBCZH_EXPECT_EQ("DT9", utils::dtoa(node["czh"]["dt9"].get<double>()), "-0.0");
    bool a = node["czh"]["int_array"].get<ContainerTest>().c == std::vector<int>{-600, 2, -9000};
    LIBCZH_EXPECT_FALSE("Node 2", !a);
    ...
    LIBCZH_EXPECT_EQ("Tostring", node.to_string(),
